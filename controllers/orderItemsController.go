...
func CreateOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		for _, orderItem := range orderItemPack.Order_items {
			...
			orderItem.Created_at, _ = time.Parse(time.RFC3339, time.Now().AddDate(0, 0, 1).Format(time.RFC3339))
			orderItem.Updated_at, _ = time.Parse(time.RFC3339, time.Now().AddDate(0, 0, 1).Format(time.RFC3339))
